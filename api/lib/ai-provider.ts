...
export class AIProvider {
     ...
     */
    async chatCompletion(request: ChatRequest, sessionId: string = 'default'): Promise<ChatResponse> {
     ...
     */
    async *streamChat(request: ChatRequest, sessionId: string = 'default'): AsyncGenerator<StreamChunk> {
        const mode = request.model || 'fast';
        const { client, deployment, modelName } = this.getClient(mode);
